...
class ICodeMongoDB {
    ...
    private static function selectDatabasse($con,$DB)
    {
        if(!ICodeMongoDB::isDatabaseExists($con, $DB))
        {
            ...
            $error =  'No such Database named: '. $DB;
            ICodeError::Log($error,'ICodeMongoDB.class','selectDatabasse','Error');
            return;
        }else{
//            echo 'Database Ok';
        }
    ...
    private static function selectCollection($db,$Collection){
        ...
        if(!ICodeMongoDB::isCollectionExists($db, $Collection)){
            ...
            $error =  "No such Collection in $db named: $Collection";
            ICodeError::Log($error,'ICodeMongoDB.class','selectCollection','Error');
            return;
        }else{
//        echo 'Collection Ok';
        }
